...
class LaplaceBase(PDENeuralNetwork):
    ...
    def ComputeLossTerms(domain, xInt, yInt, yBound, boundaryCondition):
        ...
        lossInt = tf.reduce_mean(input_tensor=tf.square(tf.add_n(gradients)))
        magnitudeInt = tf.reduce_mean(input_tensor=tf.square(tf.add_n([tf.abs(grads) for grads in gradients])))
        lossBound = tf.reduce_mean(input_tensor=tf.square(yBound - boundaryCondition))
        magnitudeBound = tf.reduce_mean(input_tensor=tf.square(boundaryCondition))
